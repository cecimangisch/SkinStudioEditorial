"""
Slide generator — 04 · Barrier Summer
...
def slide_01():
    ...
    lines = ["Your moisturiser isn't", "breaking you out."]
    spotlight = "The summer is."

    total_h = sum(th(draw, l, f_dm) + 14 for l in lines) + th(draw, spotlight, f_fr) + 60

    y = (H - total_h) // 2 - 10
    r, g, b = hex_to_rgb(INK)
    for line in lines:
        draw.text((cx(draw, line, f_dm), y), line, font=f_dm, fill=(r, g, b))
        y += th(draw, line, f_dm) + 14
...
def slide_02():
    ...
    y = 90
    r, g, b = hex_to_rgb(INK)
    head = "What heat actually does to your skin."
    draw.text((MARGIN, y), head, font=f_head, fill=(r, g, b))
    y += th(draw, head, f_head) + 20
    draw.line([(MARGIN, y), (W - MARGIN, y)], fill=(r, g, b, 102), width=1)
    y += 28

    paras = [
        "Heat increases transepidermal water loss (TEWL). Your barrier is losing water faster than usual — even if you cannot feel it.",
        "Simultaneously: sebum production increases under heat. Eccrine sweat disrupts the skin's acid mantle.",
        "The result is a compromised barrier — more reactive, more permeable, more prone to congestion.",
        "Your moisturiser did not cause this. Stopping it will not fix it.",
    ]
    for para in paras:
    ...
    y += 8
    final = "The problem is not your product. The problem is the season."
